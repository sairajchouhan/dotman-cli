...
class InitPromptCancelledError extends Error {}
...
export const init_cmd = new Command("init").description(messages.commands.init.description).action(async () => {
  ...
  let env_map: Record<string, string>;
  try {
    env_map = await prompts.group(
      Object.fromEntries(
    ...
    if (error instanceof InitPromptCancelledError) return;
    throw error;
  }
