...
export const pull_cmd = new Command("pull").action(async () => {
  ...
  if (secrets_map_client_env_keys.length > 0) {
    render_warning({
      message: `Client environment variables ${secrets_map_client_env_keys.join(", ")} found in source\nremove it from the source`,
    });
    return;
  }

  if (Object.keys(secrets_map).length === 0) {
    render_error({ message: `No secrets found in Project "${project.title}"`, exit: true });
    return;
  }
  ...
  if (apply) {
    ...
    if (write_res.isErr()) {
      render_error({ message: "Could not update env file", exit: true });
      return;
    }
