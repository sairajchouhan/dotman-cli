...
export const push_cmd = new Command("push").action(async () => {
  ...
  if (Object.keys(environment_env_map).length === 0) {
    render_error({
      ...
      suggestion: "Add some KEY=VALUE pairs to your .env file before pushing",
      exit: true,
    });
    return;
  }
  ...
  if (Object.keys(filtered_env_map).length === 0) {
    render_error({
      message: `No custom environment variables found in "${env_file_name}"`,
      suggestion: "Add some KEY=VALUE pairs apart from client environment vars to your .env file before pushing",
      exit: true,
    });
    return;
  }
